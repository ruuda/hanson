...
CREATE TABLE account
  ...
  , outcome         BIGINT       NULL REFERENCES "outcome" (id)
  ...
  , CONSTRAINT outcome_iff_outcome_share CHECK
    (
      CASE type
        WHEN 'points'         THEN (outcome IS NULL)
        WHEN 'outcome_shares' THEN (outcome IS NOT NULL)
      END
    )
...
CREATE INDEX ix_account_owner_user_id_outcome
  ON account (owner_user_id, outcome)
  WHERE owner_user_id IS NOT NULL;

CREATE INDEX ix_account_owner_market_id_outcome
  ON account (owner_market_id, outcome)
...
CREATE TABLE "transaction"
  ...
  , created_by_user_id BIGINT               NULL REFERENCES "user" (id)
  , created_at         TIMESTAMPTZ      NOT NULL DEFAULT now()
  );

CREATE INDEX ix_transaction_created_by_user_id
  ON transaction (created_by_user_id);
...
CREATE TABLE mutation
  ...
  , CONSTRAINT canonical_representation CHECK (credit_account_id < debit_account_id)
  );
